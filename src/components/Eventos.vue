    ...
    <q-item class="verdeBook">
      ...
      <q-item-side right>
        <img src="~icones/addmais32branco.png" 
             ...
             @click="$router.push('cadevento')"
             >
      </q-item-side>
    </q-item>

    <q-item class="cinzaBook"
            v-for="(evento, index) in eventosList"
            :key="index"
            >
      <q-item-side></q-item-side>
      <q-item-main class="item" @click="$router.push('cadevento')">
        <div class="no-break-row">{{evento}}</div> 
      </q-item-main>
...
export default {
  ...
  data(){
    return {
      API,
      eventos: ['Feriados', 'Campeonatos', 'Exame de Faixa'],
      search: '',
    }
  },
  computed:{
    eventosList(){
      return this.eventos.filter(row => row.toLowerCase().indexOf(this.search)>=0)
    }
  },
  methods:{
    getPessoa(pessoa){
